...
for i in range(0, 1000):
    ...
    if not soup.find("aside"):
        continue
    # to get the journal title, find the first a element inside the aside element, get its text, and strip leading and trailing whitespace
    journal_title = soup.find("aside").find("a").text.strip()
    print("processing journal:", i, "-", journal_title)
    asjp_folder = articles_folder / f"asjp-{i}-{journal_title}"
    ...
    for volume_div in volumes_divs:
        ...
        for article_div in articles_div.find_all("h4"):
            ...
            # Clean the article title by replacing multiple spaces with a single space
            cleaned_title = re.sub(r"\s+", " ", article_title)
